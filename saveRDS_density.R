...
for (year in years) {
  map_file <- paste0(map_dir, "map_density", year, ".rds")
  ...
  if (!file.exists(map_file)) {
    ...
    # NA kiszürése
    world_data_year <- world_data_year %>% filter(!is.na(PopDensity))
    
    # az adott évre az adat létrehozása, év hozzáadása
    map_year <- cartogram_cont(world_data_year, weight = "PopDensity", itermax = 5)
    map_year$Year <- as.numeric(year) # 
...
map_combined$DensityCategory <- cut(map_combined$PopDensity,
                                          breaks = c(0, 10, 20, 50, 100, 200, 300, 500, 1000, Inf),
                                          labels = c("0-10", "10-20", "20-50", "50-100", 
                                                     "100-200", "200-300", "300-500", "500-1000", "1000+"),
                                          right = FALSE, include.lowest = TRUE)

# végső file mentés
saveRDS(map_combined, file = "map_combined_density.rds")